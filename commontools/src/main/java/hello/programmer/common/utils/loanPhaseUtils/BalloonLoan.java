...
/**
 * 气球贷还款计划
 * 所谓气球贷是指n期产品中，前n-1期按照大于n的期数生成等额本息还款计划，最后一期根据剩余本金和利率生成还款计划
 * 这是一种介于等额本息和按天计息的还款方式，前期还款每期还款都有本息，最后一期以前可回本，且还款压力比等额本息小
 * 参考需求管理XQGL-801
 *【P1】融时代-气球贷项目对接
 * @author 张伯文
 * createTime 2018/5/23-18:13
 */
public class BalloonLoan {

    public static List<AstLoanPhase> createLoanPhase(LoanInfo loanInfo, Integer opId) throws Exception {
        // 起息日期
        Date interestDate = loanInfo.getInterestDate();
        // 起息日期、结息日间隔的月数
        int totalPhaseNum = loanInfo.getBalloonTerm();
        int realPhase = DateUtil.getMonthSpace(loanInfo.getInterestDate(), loanInfo.getEndInterestDate());
//        int balloonTerm=
        // 还款计划列表
        List<AstLoanPhase> loanPhaseList = new ArrayList<AstLoanPhase>(realPhase*3);
        // 借款金额
...
//                projectRateRepayCapital = projectRateRepayCapital.add(lastPhasePrincipal);
                projectRateRepayCapital=lastPhasePrincipal;
                lastPhasePrincipal = BigDecimal.ZERO;
                termRemainingPrincipal = BigDecimal.ZERO;

                manageFee = borrowerRateInterest.subtract(projectRateInterest);
